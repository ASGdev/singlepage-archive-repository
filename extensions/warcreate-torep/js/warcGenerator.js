  ...
  chrome.storage.local.get(rh, function (result) {
    ...
    if (Object.keys(responsesToConcatenate).length === 0) {
	  ...
	  chrome.tabs.query({currentWindow: true, active: true}, function(tabs){
			let u = tabs[0].url.replace(/[#\/:]/g,'!');

			var url = URL.createObjectURL(new Blob(arrayBuffers, {type: "application/warc"}));
			...
			var filenamified = u
			filenamified += "-" + Date.now() + ".warc"
			
			var title = tabs[0].title
			
			var data = new FormData()
			data.append('file', new Blob(arrayBuffers, {type: "application/warc"}), filenamified)
			data.append('seed', tabs[0].url)
			data.append('type', 'warc')
